 ...
 * 
 * NEW FEATURES:
 *  - Role-based auth (STA1-6, SUPV, EMS, MSC) + initials
...
const CONFIG = {
  ...
  AUTH: {
    ROLES: ['STA1', 'STA2', 'STA3', 'STA4', 'STA5', 'STA6', 'SUPV1', 'SUPV2', 'EMS', 'MSC', 'UNIT'],
...
function doGet(e) {
  ...
  return HtmlService.createHtmlOutput(
    ...
    '<p>ACCESS THE FRONTEND AT: <a href="https://holdenportal.com/hoscad" style="color:#4fc3f7;">holdenportal.com/hoscad</a></p>' +
    '</body></html>'
...
function _handleApiRequest(e) {
  try {
    ...
    switch(action) {
      ...
      // Maintenance
      case 'runPurge': return apiRunPurge(...params);
...
function apiUpdateIncident(token, incidentId, message, incidentType) {
  const auth = _requireAuth_(token);
  _ensureSheets_();
  const ss = _ss_();
  ...
  if (!msg && !newType) return { ok: false, error: 'MISSING INCIDENT NOTE MESSAGE.' };
  ...
  const auditMsg = newType && msg ? msg + ' [TYPE: ' + newType + ']' : newType ? '[TYPE CHANGED TO: ' + newType + ']' : msg;
...
// ============================================================
// API: Run purge + install daily trigger (SUPV only)
function apiRunPurge(token) {
  const auth = _requireAuth_(token);
  if (!auth.role.startsWith('SUPV') && auth.role !== 'STA1') return { ok: false, error: 'SUPERVISOR ACCESS REQUIRED.' };

  // Install daily trigger (idempotent — removes old trigger first)
  setupDailyPurge();

  // Run immediate purge
  const deleted = purgeOldData();

  return { ok: true, deleted: deleted, message: 'PURGE COMPLETE. ' + deleted + ' OLD ROWS DELETED. DAILY TRIGGER INSTALLED (3 AM).' };
}
